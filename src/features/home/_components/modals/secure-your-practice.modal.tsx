...
type SecureYourPracticeModalProps = {
    isOpen: boolean;
    onOpenChange: (state: boolean) => void;
};

export const SecureYourPracticeModal = ({
    isOpen,
    onOpenChange,
}: SecureYourPracticeModalProps) => {
    const { t } = useTranslation();
    const { isSmallView } = useDevice();

    const features = t('home.modals.secure_your_practice.features', {
        returnObjects: true,
    }) as { title: string; desc: string }[];
    const tabs = t('home.modals.secure_your_practice.tabs', {
        returnObjects: true,
    }) as string[];
                            ...
                            <div className="grid w-full flex-1 grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-10">
                                ...
                                <div className="flex flex-col justify-center">
                                    <h3 className="mb-4 text-xs font-extrabold tracking-[0.15em] text-primary uppercase">
                                        {t('home.modals.secure_your_practice.tag')}
                                    </h3>
                                    <h2 className="mb-8 text-4xl leading-[1.1] font-extrabold lg:text-[44px]">
                                        <span className="block text-text-default">
                                            {t(
                                                'home.modals.secure_your_practice.mainText',
                                            )}
                                        </span>
                                        <span className="block text-primary">
                                            {t(
                                                'home.modals.secure_your_practice.highlightText',
                                            )}
                                        </span>
                                    </h2>
                                    <p className="mb-8 text-base font-bold text-text-default">
                                        {t('home.modals.secure_your_practice.subtitle')}
                                    </p>
                                    <p className="max-w-md text-base leading-relaxed text-text-subdued">
                                        {t('home.modals.secure_your_practice.desc')}
                                    </p>
                                </div>
