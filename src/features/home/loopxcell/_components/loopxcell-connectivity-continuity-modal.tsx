import { Antenna, ArrowRight, Globe } from 'lucide-react';
...
export function LoopXcellConnectivityContinuityModal({
    isOpen,
    onOpenChange,
    onCTAClick,
    onSecondaryClick,
    ...
    const featuresLeft = t('home.modals.connectivity.features_left', {
        returnObjects: true,
    }) as { title: string; desc: string }[];
    const featuresRight = t('home.modals.connectivity.features_right', {
        returnObjects: true,
    }) as { title: string; desc: string }[];
    const bottomGrid = t('home.modals.connectivity.bottom_grid', {
        returnObjects: true,
    }) as { title: string; desc: string }[];
    const footerStats = t('home.modals.connectivity.footer_stats', {
        returnObjects: true,
    }) as { val: string; label: string; desc: string }[];
    const cardTitle = t('home.modals.connectivity.card_title', {
        returnObjects: true,
    }) as string[];
    const cloudList1 = t('home.modals.connectivity.cloud.list1', {
        returnObjects: true,
    }) as string[];
    const cloudList2 = t('home.modals.connectivity.cloud.list2', {
        returnObjects: true,
    }) as string[];
    ...
    const footerContent = (
        <div className="flex w-full flex-col items-center justify-between gap-4 border-t border-zinc-200 bg-[#F5F7FA] px-4 py-4 lg:flex-row lg:gap-6 lg:px-10 lg:py-6">
            {/* Left Footer */}
            <div className="flex w-full items-center gap-4 lg:w-[260px]">
                <div className="relative flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-blue-700 text-white">
                    <Antenna size={24} className="text-white" />
                </div>
                <div>
                    <h4 className="mb-0.5 text-[10px] font-bold tracking-wider whitespace-pre-line text-zinc-900 uppercase">
                        {t('home.modals.connectivity.footer_left_title')}
                    </h4>
                    <p className="text-[10px] leading-snug whitespace-pre-line text-zinc-600">
                        {t('home.modals.connectivity.footer_left_desc')}
                    </p>
                </div>
            </div>

            {/* Middle Footer Stats */}
            <div className="mx-0 flex w-full flex-wrap items-center justify-center gap-4 border-r-0 border-l-0 px-0 lg:mx-4 lg:flex-1 lg:gap-6 lg:border-r lg:border-l lg:border-zinc-200 lg:px-6">
                {footerStats.map((stat, idx) => (
                    <div key={idx} className="flex items-center gap-4 lg:gap-6">
                        {idx > 0 && <div className="h-10 w-px bg-zinc-200" />}
                        <div className="flex flex-col items-center">
                            <span className="mb-1 text-[8px] font-bold tracking-wider text-zinc-500 uppercase">
                                {stat.label}
                            </span>
                            <span className="text-xl font-bold text-blue-700 lg:text-2xl">
                                {idx === footerStats.length - 1 ? (
                                    <Globe size={24} className="my-0.5" />
                                ) : (
                                    stat.val
                                )}
                            </span>
                            <span className="mt-1 text-center text-[7px] font-bold tracking-wide whitespace-pre-line text-zinc-500 uppercase">
                                {stat.desc}
                            </span>
                        </div>
                    </div>
            ...
            <div className="flex w-full flex-col gap-2 lg:ml-auto lg:w-[240px]">
                <button
                    ...
                    onClick={onCTAClick}
                >
                    <span>{t('home.modals.connectivity.footer_btn1')}</span>
                    <ArrowRight size={14} />
                </button>
                <button
                ...
                >
                    <span>{t('home.modals.connectivity.footer_btn2')}</span>
                    <ArrowRight size={14} />
                </button>
            </div>
        </div>
                                ...
                                <div className="col-span-12 flex flex-col justify-center gap-8 lg:col-span-5">
                                    ...
                                    <div>
                                        <h3 className="mb-6 text-xl font-bold">
                                            <span className="text-zinc-900">
                                                {t(
                                                    'home.modals.connectivity.features_right_title_main',
                                                )}
                                            </span>
                                            <br />
                                            <span className="text-blue-700">
                                                {t(
                                                    'home.modals.connectivity.features_right_title_highlight',
                                                )}
                                            </span>
                                        </h3>
                                    ...
                                    <div className="mt-2 rounded-2xl border border-zinc-100 bg-background p-6 shadow-sm">
                                        ...
                                        <p className="mb-6 text-[11px] text-zinc-600">
                                            {t(
                                                'home.modals.connectivity.card_desc',
                                            )}
                                        </p>
                                        ...
                                        <div className="mb-6 flex flex-col items-center gap-6 lg:flex-row lg:gap-4">
                                            <div className="w-full flex-1 text-center lg:text-left">
                                                <h5 className="mb-2 text-[9px] font-bold tracking-wider text-blue-700">
                                                    {t(
                                                        'home.modals.connectivity.cloud.title1',
                                                    )}
                                                </h5>
                                                <p className="mb-3 text-[10px] leading-snug text-zinc-600">
                                                    {t(
                                                        'home.modals.connectivity.cloud.desc1',
                                                    )}
                                                </p>
                                            ...
                                            <div className="w-full flex-1 text-center lg:text-left">
                                                <h5 className="mb-2 text-[9px] font-bold tracking-wider text-blue-700">
                                                    {t(
                                                        'home.modals.connectivity.cloud.title2',
                                                    )}
                                                </h5>
                                                <p className="mb-3 text-[10px] leading-snug text-zinc-600">
                                                    {t(
                                                        'home.modals.connectivity.cloud.desc2',
                                                    )}
                                                </p>
                                                <ul className="inline-block space-y-1 text-left lg:block">
                                                    {cloudList2.map(
                                        ...
                                        <div className="grid grid-cols-2 gap-3 border-t border-zinc-100 pt-5 md:grid-cols-5">
                                            {bottomGrid.map((item, idx) => (
                                                <div
                                                    key={idx}
                                                    className="border-l-2 border-blue-700 py-0.5 pl-2"
                                                >
                                                    <h6 className="mb-1 text-[7px] leading-tight font-bold text-zinc-900 uppercase">
                                                        {item.title}
                                                    </h6>
